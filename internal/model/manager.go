...
func (m *Manager) BootstrapBuiltinPresets(cfg *config.Config) error {
	currentVersion := 0
	if v, ok := m.Metadata["builtin_presets_bootstrap_version"]; ok {
		fmt.Sscanf(v, "%d", &currentVersion)
	}
	if currentVersion >= BuiltinPresetBootstrapVersion {
		return nil
	}
	...
	legacyModels := map[string]ModelConfig{}
	for legacyID := range LegacyPresetIDMap {
		if model, ok := m.Models[legacyID]; ok {
			legacyModels[legacyID] = model
			delete(m.Models, legacyID)
			changed = true
		}
	}

	presetSources := map[string]map[string]ModelConfig{
		PresetDeepSeekV4Flash: {
			PresetDeepSeekV4Flash:    legacyModels["system_deepseek_chat"],
			"system_deepseek":        legacyModels["system_deepseek"],
		},
		PresetDeepSeekV4Pro: {
			PresetDeepSeekV4Pro:      legacyModels["system_deepseek_reasoner"],
			"system_deepseek_chat":   legacyModels["system_deepseek_chat"],
			"system_deepseek":        legacyModels["system_deepseek"],
		},
		PresetDoubao: {
			PresetDoubao:           legacyModels["system_doubao"],
			"system_doubao":        legacyModels["system_doubao"],
		},
	}

	for _, presetID := range []string{PresetDeepSeekV4Flash, PresetDeepSeekV4Pro, PresetDoubao} {
		if existing, ok := m.Models[presetID]; ok {
			existing.IsBuiltin = true
			m.Models[presetID] = existing
			changed = true
			continue
		}

		var source map[string]ModelConfig
		if src, ok := presetSources[presetID]; ok {
			source = src
		}
		preset := BuildBuiltinPreset(presetID, source,
			cfg.DeepSeekAPIKey, cfg.DeepSeekBaseURL,
			cfg.DoubaoAPIKey, cfg.DoubaoBaseURL, cfg)
		m.Models[presetID] = preset
		changed = true
	}

	// Update question type mappings with legacy IDs
	for qt, cfg := range m.QuestionTypeModels {
		replaced := make([]string, 0, len(cfg.Models))
		seen := map[string]bool{}
		for _, modelID := range cfg.Models {
			mappedID := LegacyPresetIDMap[modelID]
			if mappedID == "" {
				mappedID = modelID
			}
			if _, exists := m.Models[mappedID]; !exists || seen[mappedID] {
				continue
			}
			replaced = append(replaced, mappedID)
			seen[mappedID] = true
		}
		if len(replaced) != len(cfg.Models) {
			modelsCopy := make([]string, len(replaced))
			copy(modelsCopy, replaced)
			qtCfg := m.QuestionTypeModels[qt]
			qtCfg.Models = modelsCopy
	...
	defaultMappings := map[string][]string{
		config.QuestionTypeSingle:     {PresetDeepSeekV4Flash},
		config.QuestionTypeMultiple:   {PresetDeepSeekV4Pro, PresetDeepSeekV4Flash},
		config.QuestionTypeJudgement:  {PresetDeepSeekV4Flash},
		config.QuestionTypeCompletion: {PresetDeepSeekV4Flash},
		config.QuestionTypeImage:      {PresetDoubao},
	}

	for qt, defaultIDs := range defaultMappings {
		if len(m.GetQuestionTypeModels(qt)) > 0 {
			continue
		}
		available := []string{}
		for _, modelID := range defaultIDs {
			...
			if qt == config.QuestionTypeImage && !model.IsMultimodal {
				continue
			}
			available = append(available, modelID)
		}
		if len(available) > 0 {
			m.QuestionTypeModels[qt] = QuestionTypeConfig{Models: available}
			changed = true
		}
	}

	m.Metadata["builtin_presets_bootstrap_version"] = fmt.Sprintf("%d", BuiltinPresetBootstrapVersion)
	m.Metadata["builtin_presets_bootstrapped_at"] = time.Now().Format(time.RFC3339)
	changed = true
